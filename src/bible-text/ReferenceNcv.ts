...
export class ReferenceNcv implements IReferenceTools {
  ...
  private generateRegExp() {
    ...
    function g(isGb?: 1) {
      const r1 = LQ.from(
        isGb !== 1 ? BookNameConstants.CHINESE_BOOK_ABBREVIATIONS : BookNameConstants.CHINESE_BOOK_ABBREVIATIONS_GB,
      )
        .toArray()
        .join('');
  ...
  toStandard(): string {
    ...
    return LQ.from(r2)
      .select((a1) => (a1.exec === undefined ? a1.w : cvt(a1.w)))
      .toArray()
      .join('');

    function cvt(str: string): string {
      const r1 = str.replace(/；|，|~|、/g, (a1) => {
        if (a1 === '；') {
          return ';';
        }
        if (a1 === '，') {
